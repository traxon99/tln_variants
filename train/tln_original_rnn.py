# ORIGINAL TinyLidarNet TRAINING SCRIPT. DOES NOT WORK WITH 2D SIM. THIS IS PURELY FOR REFERENCE               #
# *     *     *    *     ***    *    *   ***  *    *    * * *                                         #
#  *   * *   *    * *    *  *   * *  *    *   * *  *   *                                              #
#   * *   * *    * * *   * *    *  * *    *   *  * *   *   ***                                        #
#    *     *    *     *  *   *  *    *   ***  *    *    * * *                                         #
...
# Initialize variables for seq_len, min and max speed
seq_len = 5
max_speed = 0
min_speed = 0
...
for pth in dataset_path:
    ...
    # Build sequences for this bag
    X, y = create_lidar_sequences(lidar_data, servo_data, speed_data, timestamps, sequence_length=seq_len)
    ...
    x_train_bag, x_test_bag = X[:train_samples], X[train_samples:]
    y_train_bag, y_test_bag = y[:train_samples], y[train_samples:]

    # Extend lists with train and test data
    lidar_seqs.extend(x_train_bag)
    servo_seqs.extend(y_train_bag[:, 0])
    speed_seqs.extend(y_train_bag[:, 1])

    test_lidar_seqs.extend(x_test_bag)
    test_servo_seqs.extend(y_test_bag[:, 0])
    test_speed_seqs.extend(y_test_bag[:, 1])

    print(f'\nData in {pth}:')
    print(f'Shape of Train Data --- Lidar: {len(lidar_seqs)}, Servo: {len(servo_seqs)}, Speed: {len(speed_seqs)}')
    print(f'Shape of Test Data --- Lidar: {len(test_lidar_seqs)}, Servo: {len(test_servo_seqs)}, Speed: {len(test_speed_seqs)}')

# Calculate total number of samples
total_number_samples = len(lidar_seqs)

print(f'Overall Samples = {total_number_samples}')
lidar = np.asarray(lidar_seqs)
servo = np.asarray(servo_seqs)
speed = np.asarray(speed_seqs)
speed = linear_map(speed, min_speed, max_speed, 0, 1)
test_lidar = np.asarray(test_lidar_seqs)
test_servo = np.asarray(test_servo_seqs)
test_speed = np.asarray(test_speed_seqs)
...
model = build_spatiotemporal_model(seq_len, num_lidar_range_values)
...
def evaluate_model(model_path, test_lidar, test_data):
    ...
    for lidar_data in output_lidar:
        ...
        lidar_data = np.expand_dims(lidar_data, axis=0).astype(np.float32)
