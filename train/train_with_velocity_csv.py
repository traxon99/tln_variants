...
from train_utils import huber_loss_np
...
if __name__ == '__main__':
    ...
    #Remove last element to ensure full data integrity (Some scans were not fully processed)
    ...
    print("Velocity data shape:", vel_data.shape)

    vel_data = vel_data.reshape(-1, 1).squeeze()
    ...
    x_1_train_bag, x_1_test_bag = shuffled_lidar_data[:train_samples], shuffled_lidar_data[train_samples:]
    x_2_train_bag, x_2_test_bag = shuffled_velocity_data[:train_samples], shuffled_velocity_data[train_samples:]
    ...
    lidar.extend(x_1_train_bag)
    velocity.extend(x_2_train_bag)

    servo.extend(y_train_bag[:, 0])
    speed.extend(y_train_bag[:, 1])

    test_lidar.extend(x_1_test_bag)
    test_velocity.extend(x_2_test_bag)

    test_servo.extend(y_test_bag[:, 0])
    test_speed.extend(y_test_bag[:, 1])

    print(f'\nData in {pth}:')
    print(f'Shape of Train Data --- Lidar: {len(lidar)}, Velocity: {len(velocity)} Servo: {len(servo)}, Speed: {len(speed)}')
    print(f'Shape of Test Data --- Lidar: {len(test_lidar)}, Velocity: {len(test_velocity)}, Servo: {len(test_servo)}, Speed: {len(test_speed)}')
    ...
    speed = (speed - min_speed) / (max_speed - min_speed) if max_speed != min_speed else speed


    test_lidar = np.asarray(test_lidar)
    test_velocity = np.asarray(test_velocity)

    test_servo = np.asarray(test_servo)
    test_speed = np.asarray(test_speed)
    test_speed = (test_speed - min_speed) / (max_speed - min_speed) if max_speed != min_speed else test_speed
    ...
    # Check array shapes
    assert len(lidar) == len(servo) == len(speed) == len(velocity)
    assert len(test_lidar) == len(test_servo) == len(test_speed) == len(test_velocity)
    ...
    # Check array shapes
    print(f'Train Data(lidar, velocity): {lidar.shape}, {velocity.shape}')
    ...
    num_vel_values = len(velocity)
    print(f'num_lidar_range_values: {num_lidar_range_values}')


    # Lidar Input (1D Convolutional Processing)
    lidar_input = tf.keras.Input(shape=(num_lidar_range_values, 1), name="lidar_input")
    x = tf.keras.layers.Conv1D(filters=24, kernel_size=10, strides=4, activation='relu')(lidar_input)
    ...
    # Velocity Input (Fully Connected Processing)
    velocity_input = tf.keras.Input(shape=(1,), name="velocity_input")

    # Correct concatenation of lidar and velocity inputs
    x = tf.keras.layers.Concatenate()([x, velocity_input])
    ...
    # Define the Model
    model = tf.keras.Model(inputs=[lidar_input, velocity_input], outputs=output)
    ...
    history = model.fit(
        ...
        validation_data=(
            [test_lidar, test_velocity],
            np.concatenate((test_servo[:, np.newaxis], test_speed[:, np.newaxis]), axis=1)
        )
    )
    ...
    # Evaluate test loss
    test_loss = model.evaluate([test_lidar,test_velocity], test_data)
    print(f'Overall Test Loss = {test_loss}')

    # Calculate and print overall evaluation
    y_pred = model.predict([test_lidar, test_velocity])
    hl = huber_loss_np(test_data, y_pred)
    print('\nOverall Evaluation:')
    print(f'Overall Huber Loss: {hl:.3f}')

    # Calculate and print speed evaluation
    speed_y_pred = model.predict([test_lidar, test_velocity])[:, 1]
    speed_test_loss = huber_loss_np(test_data[:, 1], speed_y_pred)
    print("\nSpeed Evaluation:")
    print(f"Speed Test Loss: {speed_test_loss}")

    # Calculate and print servo evaluation
    servo_y_pred = model.predict([test_lidar, test_velocity])[:, 0]
    servo_test_loss = huber_loss_np(test_data[:, 0], servo_y_pred)
    print("\nServo Evaluation:")
    ...
    #     print(f'Huber Loss for {model_name}: {huber_loss_np(test_data, y_pred)}\n')
