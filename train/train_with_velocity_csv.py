...
history = model.fit([lidar, velocity], np.concatenate((servo[:, np.newaxis], speed[:, np.newaxis]), axis=1),
                    epochs=num_epochs, batch_size=batch_size)#, validation_data=[test_lidar, test_velocity, test_data])

# history = model.fit([lidar, velocity], np.concatenate((servo[:, np.newaxis], speed[:, np.newaxis]), axis=1),
#                     epochs=20, batch_size=32)
history = model.fit(
    [lidar, velocity],  # Training inputs (as a list)
    np.concatenate((servo[:, np.newaxis], speed[:, np.newaxis]), axis=1),  # Training labels
    epochs=20,
    batch_size=32,
    validation_data=(
        [test_lidar, test_velocity],  # Validation inputs (also a list)
        np.concatenate((test_servo[:, np.newaxis], test_speed[:, np.newaxis]), axis=1)  # Validation labels
    )
)
